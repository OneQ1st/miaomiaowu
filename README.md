...
docker run -d \
  ...
  ghcr.io/OneQ1st/miaomiaowu:latest
```

参数说明：
- `-p 40899:40899` 将容器端口映射到宿主机，按需调整。
...
更新镜像后可执行：
```bash
docker pull ghcr.io/OneQ1st/miaomiaowu:latest
...
services:
  miaomiaowu:
    image: ghcr.io/OneQ1st/miaomiaowu:latest
    ...
    healthcheck:
      test: ["CMD", "wget", "--no-verbose", "--tries=1", "--spider", "http://localhost:40899/"]
      interval: 30s
      timeout: 3s
      start_period: 5s
      retries: 3
...
```bash
# 下载并运行安装脚本
curl -sL https://raw.githubusercontent.com/OneQ1st/miaomiaowu/main/install.sh | bash
```

安装完成后，服务将自动启动，访问 `http://服务器IP:40899` 即可。

**更新到最新版本：**
```bash
# systemd 服务更新
curl -sL https://raw.githubusercontent.com/OneQ1st/miaomiaowu/main/install.sh | sudo bash -s update
```

**卸载服务：**
```bash
# 卸载 systemd 服务（保留数据）
curl -sL https://raw.githubusercontent.com/OneQ1st/miaomiaowu/main/install.sh | sudo bash -s uninstall
...
```bash
# 一键下载安装
curl -sL https://raw.githubusercontent.com/OneQ1st/miaomiaowu/main/quick-install.sh | bash
...
```bash
# 卸载 systemd 服务（保留数据）
curl -sL https://raw.githubusercontent.com/OneQ1st/miaomiaowu/main/quick-install.sh | sudo bash -s uninstall
...
```bash
# 更新到最新版本
curl -sL https://raw.githubusercontent.com/OneQ1st/miaomiaowu/main/quick-install.sh | bash -s update
```

**Windows：**
```powershell
# 从 Releases 页面下载 mmw-windows-amd64.exe
# https://github.com/OneQ1st/miaomiaowu/releases
...
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/traffic_info.png)  
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/subscribe_url.png)  
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/probe_datasource.png)  
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/subscribe_manage.png)  
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/generate_subscribe.png)  
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/custom_proxy_group.png)  
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/node_manage.png)  
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/user_manage.png)
![image](https://github.com/OneQ1st/miaomiaowu/blob/main/screenshots/system_settings.png)
</details>
...
[![Star History Chart](https://api.star-history.com/svg?repos=OneQ1st/miaomiaowu&type=date&legend=top-left)](https://www.star-history.com/#OneQ1st/miaomiaowu&type=date&legend=top-left)
...
- 问题反馈：[GitHub Issues](https://github.com/OneQ1st/miaomiaowu/issues)
- 功能建议：[GitHub Discussions](https://github.com/OneQ1st/miaomiaowu/discussions)
## 更新日志
<details>
...
- 🌈 增加覆写脚本功能[#33](https://github.com/OneQ1st/miaomiaowu/issues/33)
- 🛠️ fix:stash dns nameserver-policy 配置错误
### v0.6.6 (2026-05-07)
- 🌈 静态资源使用相对路径支持子路径代理[#78](https://github.com/OneQ1st/miaomiaowu/issues/78)
- 🌈 支持naive与mieru协议[#76](https://github.com/OneQ1st/miaomiaowu/issues/76)
- 🛠️ fix:编辑订阅信息溢出屏幕
- 🛠️ fix:hy2 sni和server相同时没有设置sni
- 🛠️ fix:stash dns配置优化
- 🛠️ fix:hy2 password没有urldecode[#77](https://github.com/OneQ1st/miaomiaowu/issues/77)
### v0.6.5 (2026-04-25)
- 🛠️ fix:节点名称过滤的默认表达式无法删除[#70](https://github.com/OneQ1st/miaomiaowu/issues/70)
- 🛠️ fix:订阅过期时间丢失
### v0.6.4 (2026-04-19)
- 🌈 新增订阅流量设置
- 🌈 简化探针绑定服务器流量设置
...
- 🛠️ fix:编辑模板时手机端显示问题
- 🛠️ fix:tls类型代理默认sni为空时优先取host[#65](https://github.com/OneQ1st/miaomiaowu/issues/65)
### v0.5.8 (2026-03-23)
- 🌈 模板v3支持yaml变量
- 🌈 节点详情支持yaml展示
- 🌈 模板v3支持icon与hidden配置
...
- 🛠️ fix:自定义连接重复的问题
- 🛠️ fix:解析ss协议时去除ipv6地址的括号[#60](https://github.com/OneQ1st/miaomiaowu/issues/60)
### v0.5.4 (2026-03-05)
- 🌈 优化更新日志与debug功能显示
- 🌈 新增设置自定义订阅连接的功能(Beta)
- 🌈 订阅流量和过期信息节点展示开关
...
- 🌈 增加外部订阅节点名称过滤
- 🛠️ fix:tuic协议skip-cert-verify添加默认值[#56](https://github.com/OneQ1st/miaomiaowu/issues/56)
- 🛠️ fix:规则引用代理节点时误判
...
- 🛠️ fix:loon订阅缺少obfs参数[#51](https://github.com/OneQ1st/miaomiaowu/issues/51)
### v0.4.7 (2026-01-20)
- 🌈 增加两个防泄漏dns配置模板
- 🌈 支持clashtosurge配置模板转换
- 🛠️ fix:订阅过期默认配置文件错误[#48](https://github.com/OneQ1st/miaomiaowu/issues/48)
### v0.4.6 (2026-01-17)
- 🛠️ fix代理节点重复时自动去重
- 🛠️ fix:国内服务没有把DIRECT放在第一位
- 🛠️ fix:规则不允许修改类型[#40](https://github.com/OneQ1st/miaomiaowu/issues/40)
- 🛠️ fix:surge loon 协议sni参数处理
- 🛠️ fix:qx vless协议缺少tls-host参数
- 🛠️ fix:代理组配置校验缺少filter与Include-all
### v0.4.5 (2026-01-17)
- 🌈 增加简单订阅过期功能
- 🛠️ fix:代理集合的节点未更新[#49](https://github.com/OneQ1st/miaomiaowu/issues/49)
- 🛠️ fix:订阅流量统计错误[#50](https://github.com/OneQ1st/miaomiaowu/issues/50)
...
- 🌈 外部订阅地址支持修改
- 🌈 使用虚拟dom减少页面卡顿
- 🌈 合并生成订阅与添加代理组的预置代理组数据，代理组模板支持从github拉取 [proxy-groups.json](https://raw.githubusercontent.com/OneQ1st/miaomiaowu/refs/heads/main/proxy_groups/proxy-groups.default.json)
- 🌈 添加代理组时支持选择emoji
- 🌈 生成订阅页面节点支持排序
- 🌈 优化代理组节点拖动性能
...
- 🛠️ fix:手机端不显示临时订阅按钮
- 🛠️ fix:节点名称空格编码成+号[#31](https://github.com/OneQ1st/miaomiaowu/issues/31)
### v0.2.8 (2025-12-14)
- 🌈 支持导出带规则的stash配置
...
- 🌈 增加手动同步外部订阅按钮[#23](https://github.com/OneQ1st/miaomiaowu/issues/23)
- 🌈 调整自动选择的代理组属性顺序
- 🌈 增加自定义规则同步开关[#23](https://github.com/OneQ1st/miaomiaowu/issues/23)
- 🛠️ fix:修复拖动节点时光标闪烁
- 🛠️ fix:修复一系列yaml操作产生的双引号、属性顺序错误问题
### v0.2.1 (2025-11-28)
- 🌈 规则引用了不存在的代理组时支持替换为任意代理组
- 🛠️ fix:节点列表快速复制节点为uri格式时缺少sni参数
- 🛠️ fix:【BUG】端口配置莫名出现双引号[#22](https://github.com/OneQ1st/miaomiaowu/issues/22)
- 🛠️ fix:处理yaml时没有保持原始格式[#22](https://github.com/OneQ1st/miaomiaowu/issues/22)
### v0.2.0 (2025-11-27)
- 🌈 可用节点支持名称与标签筛选[#21](https://github.com/OneQ1st/miaomiaowu/issues/21)
- 🛠️ fix:订阅管理节点操作后，负载均衡相关参数消失[#22](https://github.com/OneQ1st/miaomiaowu/issues/22) 
### v0.1.9 (2025-11-26)
- 🛠️ fix:调整代理组的节点顺序时不再重新加载整个代理组列表跳回顶部  
- 🛠️ fix:外部订阅节点信息变更一次后丢失外部订阅关联  
- 🛠️ fix:short-id为""时，订阅种显示为空  
- 🛠️ fix:(BUG) 代理组的属性顺序错误[#19](https://github.com/OneQ1st/miaomiaowu/issues/19)  
### v0.1.8 (2025-11-25)
- 🌈 节点批量重命名[#15](https://github.com/OneQ1st/miaomiaowu/issues/15)
- 🛠️ fix:节点删除后订阅里删不全，会留几个没有删掉[#17](https://github.com/OneQ1st/miaomiaowu/issues/17)
- 🛠️ fix:(BUG) 某些情况下Vless节点的Short-id到订阅里会改变成指数[#18](https://github.com/OneQ1st/miaomiaowu/issues/18)
### v0.1.7 (2025-11-24)
